...
function newProject() {
	...
	noteText.value = "";
	noteText.focus();
	projectTitleText.textContent = "Sin título";
...
function projectToXml(project, indent = "\t") {
	...
	lines.push(
		`${indent}\t\t<stringCount>${project.settings.stringCount}</stringCount>`
	);
	lines.push(
